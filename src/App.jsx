...
const STATUS = {
  ...
  pago:     {label:"Pago",     color:"#A78BFA",bg:"rgba(167,139,250,0.12)",dot:"#A78BFA",icon:"💰"},
...
const seedData = u0 => ({
  ...
  profile:{...BLANK_PROFILE},activity:[],fotos:{},notificacoes:[],estoque:[],despesas:[],receitas:[],funil:[],
});
...
function AppShell({user:initialUser,onLogout}){
  ...
  const isSaving=useRef(false);  // evita sobrescrever dados locais com fetch durante save
  const lastSaved=useRef(0);

  const ARRAY_KEYS=new Set(["budgets","clients","templates","activity","notificacoes","estoque","despesas","receitas","funil","chatMsgs"]);

  // Normaliza row do banco → estado local
  const normalizeRow=useCallback(row=>{
    ...
    return{
      ...
      fotos:d.fotos||{},
      notificacoes:Array.isArray(d.notificacoes)?d.notificacoes:[],
      estoque:Array.isArray(d.estoque)?d.estoque:[],
      despesas:Array.isArray(d.despesas)?d.despesas:[],
      receitas:Array.isArray(d.receitas)?d.receitas:[],
      chatMsgs:Array.isArray(d.chatMsgs)?d.chatMsgs:[],
      orcCounter:d.orcCounter||1,
      funil:Array.isArray(d.funil)?d.funil:[],
    };
  },[]);

  // Carrega dados do Supabase ao montar
  useEffect(()=>{
    supabase.from("user_data").select("*").eq("user_id",initialUser.id).maybeSingle()
      .then(({data:row})=>{
        if(row&&row.data){
          setDataState(normalizeRow(row));
        } else {
          const seed=seedData(initialUser.id);
          const init={...seed,orcCounter:1,chatMsgs:[],receitas:[],despesas:[],funil:[]};
          supabase.from("user_data").upsert({user_id:initialUser.id,data:init},{onConflict:"user_id"})
            .then(()=>setDataState(init));
        }
        setLoading(false);
      });
  },[initialUser.id,normalizeRow]);

  // ── Sync em tempo real: polling 3s + Realtime Supabase ─────────────────
  useEffect(()=>{
    if(loading)return;

    // Polling a cada 3 segundos
    const poll=setInterval(()=>{
      // Não sobrescreve se acabou de salvar (janela de 2s)
      if(Date.now()-lastSaved.current<2000)return;
      supabase.from("user_data").select("*").eq("user_id",initialUser.id).maybeSingle()
        .then(({data:row})=>{
          const norm=normalizeRow(row);
          if(norm)setDataState(norm);
        });
    },3000);

    // Realtime: ouve mudanças na tabela user_data
    const channel=supabase.channel(`user_data_${initialUser.id}`)
      .on("postgres_changes",{
        event:"UPDATE",schema:"public",table:"user_data",
        filter:`user_id=eq.${initialUser.id}`
      },payload=>{
        if(Date.now()-lastSaved.current<2000)return;
        const norm=normalizeRow(payload.new);
        if(norm)setDataState(norm);
      })
      .subscribe();
  ...
  },[loading,initialUser.id,normalizeRow]);

  // Atualiza user (plano) a cada 30s
  useEffect(()=>{
    const refresh=()=>{
  ...
  const saveToDb=useCallback((d)=>{
    ...
    saveTimeout.current=setTimeout(()=>{
      ...
      supabase.from("user_data").upsert({user_id:user.id,data:safe},{onConflict:"user_id"})
  ...
  const patch=k=>fn=>setData(d=>{
    const cur=d[k]!=null?d[k]:(ARRAY_KEYS.has(k)?[]:undefined);
...
function App({user,data,patch,themeP,themeA,onLogout}){
  const {budgets=[],clients=[],templates=[],profile={},activity=[],fotos={},notificacoes=[],estoque=[],despesas=[],receitas=[],funil=[],orcCounter=1,chatMsgs=[]}=data;
  ...
  const setReceitas=patch("receitas");
  ...
  const saveBudget=f=>{
    ...
    if(f.clientName&&!(Array.isArray(clients)?clients:[]).find(c=>c.id===f.clientId))
  ...
  const markPago=(id,pagamento)=>{
    setBudgets(bs=>bs.map(b=>b.id===id?{...b,status:"pago",pagamento}:b));
    showToast("💰 Pagamento registrado!");
    addNotif(`Pagamento registrado: orçamento ${id.slice(0,8)}`,"💰");
  };
  ...
  const saveClient=c=>{
    ...
    setClients(cs=>{const a=Array.isArray(cs)?cs:[];return a.find(x=>x.id===c.id)?a.map(x=>x.id===c.id?c:x):[...a,{...c,id:uid(),userId:user.id}];});showToast("Cliente salvo ✓");setModal(null);
  };
  const delClient=id=>{setClients(cs=>cs.filter(c=>c.id!==id));setModal(null);showToast("Removido","warn");};
  const saveTpl=t=>{setTemplates(ts=>{const a=Array.isArray(ts)?ts:[];return a.find(x=>x.id===t.id)?a.map(x=>x.id===t.id?t:x):[...a,{...t,id:uid(),userId:user.id}];});showToast("Template salvo ✓");setModal(null);};
  ...
  const stats=useMemo(()=>{
    ...
    const pg=budgets.filter(b=>b.status==="pago");
    const pend=budgets.filter(b=>b.status==="pendente"||b.status==="enviado");
    const apPg=[...ap,...pg];
    return{total:budgets.length,aprovado:ap.length,pago:pg.length,pendente:pend.length,recusado:budgets.filter(b=>b.status==="recusado").length,
      receita:apPg.reduce((s,b)=>s+b.total,0),
      receitaPaga:pg.reduce((s,b)=>s+(b.pagamento?.valor||b.total),0),
      pipeline:pend.reduce((s,b)=>s+b.total,0),
      taxa:budgets.length?Math.round(apPg.length/budgets.length*100):0,
      tick:apPg.length?Math.round(apPg.reduce((s,b)=>s+b.total,0)/apPg.length):0};
  },[budgets]);
  ...
  const pageContent=(
    <main style={{flex:1,overflowY:"auto",background:"#0A0E1A",paddingBottom:isMobile?80:0}}>
      ...
      {page==="financeiro"    &&(isLocked("financeiro")?<PageLocked feature="financeiro" plan={plan} planInfo={planInfo} themeP={themeP}/>:<PageFinanceiro budgets={budgets} despesas={despesas} setDespesas={setDespesas} receitas={receitas} setReceitas={setReceitas} profile={profile} themeP={themeP} themeA={themeA} showToast={showToast}/>)}
  ...
  const modais=(
    <React.Fragment>
      ...
      {modal?.type==="detail"   &&<ModalDetail data={modal.data} onClose={()=>setModal(null)} setStatus={setStatus} markPago={markPago} sendWA={sendWA} onEdit={d=>setModal({type:"budget",data:d})} onDelete={delBudget} themeP={themeP} setModal={setModal} profile={profile}/>}
...
function PageConfig({profile,setProfile,user,themeP,themeA,showToast}){
  ...
  const TABS=[{id:"dados",lbl:"👤 Dados"},{id:"visual",lbl:"🎨 Visual"},{id:"fiscal",lbl:"🧾 Fiscal"},{id:"orcamento",lbl:"📄 Orçamento"},{id:"pix",lbl:"💚 PIX"},{id:"social",lbl:"🌐 Links"}];
  ...
  return(
    <div style={S.page}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr",gap:14,maxWidth:680}}>
        ...
        {tab==="pix"&&(
          <React.Fragment>
            <Card title="💚 Chave PIX para Cobrança">
              <div style={{fontSize:12,color:"#64748B",marginBottom:16}}>
                Sua chave PIX aparecerá nos orçamentos quando você usar o botão <b style={{color:"#22D3A0"}}>Cobrar</b>. Facilita o pagamento pelo cliente.
              </div>
              <div style={{display:"flex",flexDirection:"column",gap:12}}>
                <FL label="Tipo da Chave PIX">
                  <select style={S.sel} value={f.pixTipo||""} onChange={e=>set("pixTipo",e.target.value)}>
                ...
                <FL label="Chave PIX">
                  <input style={S.inp} value={f.pixChave||""} onChange={e=>set("pixChave",e.target.value)}
                    placeholder={f.pixTipo==="cpf"?"000.000.000-00 ou 00.000.000/0001-00":f.pixTipo==="email"?"seu@email.com":f.pixTipo==="telefone"?"+5511999999999":f.pixTipo==="aleatoria"?"xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx":"Informe o tipo acima primeiro"}/>
                </FL>
                <FL label="Nome do Favorecido (aparece no PDF)">
                  <input style={S.inp} value={f.pixNome||""} onChange={e=>set("pixNome",e.target.value)} placeholder={f.name||"Seu nome completo"}/>
                </FL>
                <FL label="Banco (opcional)">
                  <input style={S.inp} value={f.pixBanco||""} onChange={e=>set("pixBanco",e.target.value)} placeholder="Ex: Nubank, Itaú, Bradesco…"/>
                </FL>
              </div>
            </Card>
            {f.pixChave&&(
              <div style={{padding:"16px 18px",background:"rgba(34,211,160,.08)",border:"1px solid rgba(34,211,160,.25)",borderRadius:14,display:"flex",alignItems:"center",gap:14}}>
                <div style={{fontSize:28}}>💚</div>
                <div>
                  <div style={{fontSize:13,fontWeight:700,color:"#22D3A0",marginBottom:2}}>PIX configurado!</div>
                  <div style={{fontSize:12,color:"#94A3B8"}}>{f.pixNome||f.name} · {f.pixTipo?.toUpperCase()} · {f.pixChave}</div>
                  {f.pixBanco&&<div style={{fontSize:11,color:"#64748B",marginTop:2}}>{f.pixBanco}</div>}
                </div>
              </div>
            )}
            <div style={{padding:"12px 14px",background:"rgba(129,140,248,.06)",border:"1px solid rgba(129,140,248,.15)",borderRadius:12,fontSize:11,color:"#64748B",lineHeight:1.6}}>
              <div style={{fontWeight:700,color:themeP,marginBottom:4}}>💡 Como funciona</div>
              Ao abrir um orçamento e clicar em <b style={{color:"#22D3A0"}}>Cobrar via PIX</b>, o sistema gera automaticamente um PDF com os dados do serviço e sua chave PIX em destaque para o cliente efetuar o pagamento.
            </div>
          </React.Fragment>
        )}
...
/* ═══ GERAR ORÇAMENTO PDF ═══════════════════════════════════════════════ */
function gerarOrcamentoPDF(data, profile, themeP, themeA, comPix=false){
  const sub=calcSub(data.items||[]);
  const fmtVal=v=>"R$ "+Number(v||0).toLocaleString("pt-BR",{minimumFractionDigits:2});
  const fmtData=d=>d?new Date(d+"T12:00:00").toLocaleDateString("pt-BR"):"—";
  const st=STATUS[data.status]||STATUS.rascunho;

  const itensHtml=(data.items||[]).map((it,i)=>`
    <tr style="background:${i%2===0?"#fff":"#F8FAFC"}">
      <td style="padding:9px 12px;border:1px solid #E2E8F0;font-size:13px;color:#1E293B">${it.desc||"—"}</td>
      <td style="padding:9px 12px;border:1px solid #E2E8F0;font-size:13px;text-align:center;color:#475569">${it.qty||1}</td>
      <td style="padding:9px 12px;border:1px solid #E2E8F0;font-size:13px;text-align:center;color:#475569">${it.unit||"un"}</td>
      <td style="padding:9px 12px;border:1px solid #E2E8F0;font-size:13px;text-align:right;color:#475569">${fmtVal(it.price)}</td>
      <td style="padding:9px 12px;border:1px solid #E2E8F0;font-size:13px;text-align:right;font-weight:700;color:${themeP}">${fmtVal((parseFloat(it.qty)||0)*(parseFloat(it.price)||0))}</td>
    </tr>`).join("");

  const pixBlock = comPix && profile.pixChave ? `
    <div style="margin-top:24px;padding:20px 24px;background:linear-gradient(135deg,rgba(34,211,160,.08),rgba(16,185,129,.04));border:2px solid rgba(34,211,160,.35);border-radius:14px">
      <div style="display:flex;align-items:center;gap:10;margin-bottom:14px">
        <div style="width:36px;height:36px;background:linear-gradient(135deg,#22D3A0,#10B981);border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:18px;flex-shrink:0">💚</div>
        <div>
          <div style="font-size:15px;font-weight:900;color:#0F172A">Pagamento via PIX</div>
          <div style="font-size:11px;color:#64748b">Copie a chave abaixo e realize o pagamento</div>
        </div>
      </div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
        <div style="padding:12px 14px;background:#fff;border-radius:10px;border:1px solid rgba(34,211,160,.3)">
          <div style="font-size:10px;color:#94a3b8;font-weight:700;text-transform:uppercase;letter-spacing:.5px;margin-bottom:4px">Tipo da Chave</div>
          <div style="font-size:14px;font-weight:800;color:#0F172A">${({cpf:"CPF / CNPJ",email:"E-mail",telefone:"Telefone",aleatoria:"Chave Aleatória"}[profile.pixTipo]||profile.pixTipo||"PIX").toUpperCase()}</div>
        </div>
        <div style="padding:12px 14px;background:#fff;border-radius:10px;border:1px solid rgba(34,211,160,.3)">
          <div style="font-size:10px;color:#94a3b8;font-weight:700;text-transform:uppercase;letter-spacing:.5px;margin-bottom:4px">Favorecido</div>
          <div style="font-size:13px;font-weight:700;color:#0F172A">${profile.pixNome||profile.name||"—"}</div>
          ${profile.pixBanco?`<div style="font-size:11px;color:#64748b;margin-top:2px">${profile.pixBanco}</div>`:""}
        </div>
      </div>
      <div style="margin-top:12px;padding:14px 16px;background:#fff;border-radius:10px;border:2px dashed rgba(34,211,160,.5);text-align:center">
        <div style="font-size:10px;color:#94a3b8;font-weight:700;text-transform:uppercase;letter-spacing:.5px;margin-bottom:6px">Chave PIX</div>
        <div style="font-size:16px;font-weight:900;color:#059669;letter-spacing:.5px;word-break:break-all">${profile.pixChave}</div>
      </div>
      <div style="margin-top:10px;padding:10px 12px;background:rgba(245,158,11,.08);border-radius:8px;border:1px solid rgba(245,158,11,.25);font-size:11px;color:#92400e">
        ⚠️ Ao realizar o pagamento, informe o número do orçamento <b>${data.num}</b> como identificação.
      </div>
    </div>` : "";

  const totaisHtml=`
    ${data.discount>0?`<tr><td colspan="4" style="padding:7px 12px;border:1px solid #E2E8F0;text-align:right;font-size:12px;color:#64748b">Subtotal</td><td style="padding:7px 12px;border:1px solid #E2E8F0;text-align:right;font-size:12px;color:#64748b">${fmtVal(sub)}</td></tr>
    <tr><td colspan="4" style="padding:7px 12px;border:1px solid #E2E8F0;text-align:right;font-size:12px;color:#EF4444">Desconto (${data.discount}%)</td><td style="padding:7px 12px;border:1px solid #E2E8F0;text-align:right;font-size:12px;color:#EF4444">-${fmtVal(sub*data.discount/100)}</td></tr>`:""}
    ${data.tax>0?`<tr><td colspan="4" style="padding:7px 12px;border:1px solid #E2E8F0;text-align:right;font-size:12px;color:#3B82F6">Impostos (${data.tax}%)</td><td style="padding:7px 12px;border:1px solid #E2E8F0;text-align:right;font-size:12px;color:#3B82F6">+${fmtVal(sub*(1-data.discount/100)*data.tax/100)}</td></tr>`:""}
    <tr style="background:${themeP}12"><td colspan="4" style="padding:12px;border:1px solid ${themeP}30;text-align:right;font-weight:900;font-size:15px;color:#0F172A">TOTAL</td><td style="padding:12px;border:1px solid ${themeP}30;text-align:right;font-weight:900;font-size:18px;color:${themeP}">${fmtVal(data.total)}</td></tr>`;

  const logoHtml=profile.logo&&profile.showLogo!==false
    ?`<img src="${profile.logo}" style="height:48px;max-width:130px;object-fit:contain;background:rgba(255,255,255,.15);border-radius:8px;padding:4px 8px"/>`
    :`<div style="width:48px;height:48px;border-radius:12px;background:rgba(255,255,255,.25);display:flex;align-items:center;justify-content:center;font-size:22px">⚡</div>`;
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    @media print{body{background:#fff}.no-print{display:none!important}.page{max-width:100%;box-shadow:none}}
  ...
  <div style="background:linear-gradient(135deg,${themeP},${themeA});padding:24px 32px;display:flex;justify-content:space-between;align-items:center">
    <div style="display:flex;align-items:center;gap:14">
      ...
      <div>
        ...
        ${profile.profession?`<div style="font-size:12px;color:rgba(255,255,255,.8)">${profile.profession}</div>`:""}
        ${profile.tagline?`<div style="font-size:10px;color:rgba(255,255,255,.7);font-style:italic">${profile.tagline}</div>`:""}
      </div>
    </div>
    <div style="text-align:right">
      <div style="font-size:22px;font-weight:900;color:#fff">${data.num}</div>
      <div style="font-size:12px;color:rgba(255,255,255,.8)">${fmtData(data.date)}</div>
      <div style="margin-top:5px;display:inline-block;padding:3px 12px;border-radius:20px;background:rgba(255,255,255,.2);color:#fff;font-size:11px;font-weight:700;backdrop-filter:blur(4px)">${st.icon} ${st.label}</div>
    </div>
  </div>
  ${profile.headerNote?`<div style="background:${themeP}12;padding:8px 32px;font-size:11px;color:${themeP};font-weight:500;border-bottom:1px solid ${themeP}20">${profile.headerNote}</div>`:""}

  <div style="padding:28px 32px">
    <!-- CLIENTE + SERVIÇO -->
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:20;margin-bottom:22px">
      <div style="padding:16px;background:#F8FAFC;border-radius:12px;border-left:4px solid ${themeP}">
        ...
        <div style="font-size:16px;font-weight:800;color:#0F172A">${data.clientName}</div>
        ${data.phone?`<div style="font-size:12px;color:#64748b;margin-top:3px">📱 ${data.phone}</div>`:""}
      </div>
      <div style="padding:16px;background:#F8FAFC;border-radius:12px;border-left:4px solid ${themeA}">
        ...
        <div style="font-size:15px;font-weight:800;color:#0F172A">${data.title}</div>
        <div style="font-size:11px;color:#64748b;margin-top:3px">${data.category} · Válido ${data.validity} dias</div>
      </div>
    </div>

    ${data.desc?`<div style="padding:10px 14px;background:${themeP}08;border-radius:10px;border-left:3px solid ${themeP};font-size:12px;color:#475569;margin-bottom:20px">${data.desc}</div>`:""}

    <!-- ITENS -->
    <table style="width:100%;border-collapse:collapse;margin-bottom:20px">
      <thead>
        <tr style="background:${themeP}12">
          ${["Descrição","Qtd","Unid.","Valor unit.","Total"].map(h=>`<th style="padding:10px 12px;border:1px solid ${themeP}20;font-size:11px;font-weight:700;color:${themeP};text-align:left;text-transform:uppercase;letter-spacing:.5px">${h}</th>`).join("")}
        </tr>
      </thead>
      <tbody>${itensHtml}</tbody>
      <tfoot>${totaisHtml}</tfoot>
    </table>

    ${data.notes?`<div style="margin-bottom:16px;padding:10px 14px;background:#FFFBEB;border-radius:10px;border:1px solid #FDE68A;font-size:12px;color:#92400E">📝 ${data.notes}</div>`:""}
    ${data.endereco?`<div style="margin-bottom:16px;padding:8px 14px;background:#EFF6FF;border-radius:10px;border:1px solid #BFDBFE;font-size:12px;color:#1E40AF">📍 ${[data.endereco,data.enderecoComp,data.endereocoCidade,data.enderecoCep].filter(Boolean).join(", ")}</div>`:""}
    ${data.assinatura?`<div style="margin-bottom:16px;padding:10px 14px;background:#DCFCE7;border-radius:10px;border:1px solid #BBF7D0"><div style="font-size:11px;color:#166534;font-weight:700;margin-bottom:6px">✅ Assinado por ${data.assinadoPor||data.clientName} em ${data.assinadoEm}</div><img src="${data.assinatura}" style="max-height:50px;object-fit:contain"/></div>`:""}

    ${pixBlock}

    <!-- FOOTER -->
    <div style="margin-top:28px;padding-top:16px;border-top:1px solid #E2E8F0;display:flex;justify-content:space-between;align-items:flex-end;flex-wrap:wrap;gap:8px">
      <div>
        ${profile.footerNote?`<div style="font-size:11px;color:#94A3B8;margin-bottom:4px;font-style:italic">${profile.footerNote}</div>`:""}
      ...
      <div style="font-size:11px;color:#94A3B8">${profile.signOff||""}</div>
    </div>
  </div>
</div>
  ...
  return(
    <Overlay onClose={onClose} wide>
      ...
      <div style={{display:"flex",gap:8,marginTop:14,justifyContent:"flex-end",flexWrap:"wrap"}}>
        ...
        <button style={{...S.prim,background:"#3B82F6",color:"#fff"}} onClick={()=>gerarOrcamentoPDF(data,profile,themeP,themeA,false)}>📄 Exportar PDF</button>
        {profile.pixChave&&(
          <button style={{...S.prim,background:"linear-gradient(135deg,#22D3A0,#10B981)",color:"#0F172A"}} onClick={()=>gerarOrcamentoPDF(data,profile,themeP,themeA,true)}>💚 Cobrar via PIX</button>
        )}
...
function PageEstoque({estoque,setEstoque,themeP,themeA,showToast}){
  ...
  const saveItem=item=>{setEstoque(es=>{const a=Array.isArray(es)?es:[];return a.find(x=>x.id===item.id)?a.map(x=>x.id===item.id?item:x):[...a,{...item,id:uid()}];});setModal(null);showToast("Item salvo ✓");};
...
function exportDREPDF(budgets,despesas,receitas,mes,ano,profile,themeP,themeA){
  const meses=["Janeiro","Fevereiro","Março","Abril","Maio","Junho","Julho","Agosto","Setembro","Outubro","Novembro","Dezembro"];
  const label=`${meses[mes]} / ${ano}`;
  ...
  const rMes=(receitas||[]).filter(r=>{const dt=new Date(r.date);return dt.getMonth()===mes&&dt.getFullYear()===ano;});
  const receitaExtra=rMes.reduce((s,r)=>s+(r.valor||0),0);
  const receitaBruta=receitaOrc+receitaExtra;
...
function PageFinanceiro({budgets,despesas,setDespesas,receitas=[],setReceitas,profile,themeP,themeA,showToast}){
  ...
  const fluxo6=Array.from({length:6},(_,i)=>{
    ...
    const recOrc=budgets.filter(b=>{const bd=new Date(b.createdAt||b.date);return bd.getMonth()===m&&bd.getFullYear()===a&&b.status==="aprovado"||b.status==="pago";}).reduce((s,b)=>s+b.total,0);
    const recExtra=(receitas||[]).filter(r=>{const rd=new Date(r.date);return rd.getMonth()===m&&rd.getFullYear()===a;}).reduce((s,r)=>s+(r.valor||0),0);
    const rec=recOrc+recExtra;
  ...
  const saveDespesa=d=>{
    setDespesas(ds=>{const a=Array.isArray(ds)?ds:[];return a.find(x=>x.id===d.id)?a.map(x=>x.id===d.id?d:x):[...a,{...d,id:uid()}];});
    setModal(null);showToast("Despesa salva ✓");
  };
  const delDespesa=id=>{setDespesas(ds=>(Array.isArray(ds)?ds:[]).filter(x=>x.id!==id));setModal(null);showToast("Removida","warn");};
  const saveReceita=r=>{setReceitas(rs=>{const arr=Array.isArray(rs)?rs:[];return arr.find(x=>x.id===r.id)?arr.map(x=>x.id===r.id?r:x):[...arr,{...r,id:uid()}];});setModal(null);showToast("Receita salva ✓");};
  const delReceita=id=>{setReceitas(rs=>{const arr=Array.isArray(rs)?rs:[];return arr.filter(x=>x.id!==id);});setModal(null);showToast("Removida","warn");};
    ...
    <div style={S.page}>
      <PHead title="💰 Financeiro" sub={`${meses[mes]} ${ano} · Fluxo de caixa e DRE`}>
        <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
          ...
          <button style={{...S.prim,fontSize:12,padding:"7px 14px",background:"#22D3A0",color:"#0A0E1A"}} onClick={()=>setModal({type:"receita",data:null})}>+ Receita</button>
          ...
          <button style={{...S.prim,fontSize:12,padding:"7px 14px",background:"linear-gradient(135deg,#EF4444,#DC2626)"}} onClick={()=>exportDREPDF(budgets,despesas,receitas,mes,ano,profile,themeP,themeA)}>📄 DRE PDF</button>
        </div>
      </PHead>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:12}}>
        {[
          ...
          <div key={k.lbl} style={{...S.card,padding:16,background:k.bg,border:`1px solid ${k.color}${k.bold?"40":"20"}`}}>
            ...
            <div style={{fontSize:k.bold?22:18,fontWeight:900,color:k.color}}>{k.val}</div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12,marginBottom:20}}>
        {[
          {ico:"🔴",lbl:"Total Despesas",val:fmtBRL(totalDesp),sub:`${dMes.length} lançamento(s)`,color:"#F87171",bg:"rgba(248,113,113,0.08)"},
          {ico:"⭐",lbl:"Lucro Líquido",val:fmtBRL(lucroLiq),sub:lucroLiq>=0?"positivo":"negativo",color:lucroLiq>=0?"#22D3A0":"#F87171",bg:lucroLiq>=0?"rgba(34,211,160,0.08)":"rgba(248,113,113,0.08)",bold:true},
          {ico:"📊",lbl:"Margem de Lucro",val:`${margem}%`,sub:"sobre receita total",color:themeP,bg:`${themeP}10`},
        ].map(k=>(
          <div key={k.lbl} style={{...S.card,padding:16,background:k.bg,border:`1px solid ${k.color}${k.bold?"40":"20"}`}}>
            <div style={{fontSize:18,marginBottom:4}}>{k.ico}</div>
            <div style={{fontSize:10,color:"#64748B",fontWeight:700,textTransform:"uppercase",letterSpacing:.5,marginBottom:3}}>{k.lbl}</div>
      ...
      <div style={{display:"flex",gap:4,marginBottom:16,borderBottom:"1px solid #1E293B",paddingBottom:0}}>
        {[{id:"fluxo",lbl:"📈 Fluxo de Caixa"},{id:"despesas",lbl:"🔴 Despesas"},{id:"receitas",lbl:"💚 Receitas"},{id:"dre",lbl:"📊 DRE"}].map(t=>(
        ...
        <div>
          ...
          {bMes.length>0&&(
            <div style={{marginTop:20}}>
              <div style={{fontSize:12,color:"#64748B",fontWeight:700,marginBottom:12,textTransform:"uppercase",letterSpacing:.5}}>Orçamentos de {meses[mes]}</div>
              <div style={{...S.card,padding:0,overflow:"hidden"}}>
                <table style={{width:"100%",borderCollapse:"collapse"}}>
                  <thead><tr style={{background:"#0F172A"}}>
                    {["Nº","Cliente","Status","Total","Ações"].map(h=><th key={h} style={{padding:"10px 14px",fontSize:11,fontWeight:700,color:"#475569",textAlign:"left",borderBottom:"1px solid #1E293B",textTransform:"uppercase"}}>{h}</th>)}
                  </tr></thead>
                  <tbody>{bMes.sort((a,b)=>new Date(b.createdAt||b.date)-new Date(a.createdAt||a.date)).map(b=>{
                    const st=STATUS[b.status]||STATUS.rascunho;
                    return(
                      <tr key={b.id} style={{borderBottom:"1px solid #0F172A"}} className="trow">
                        <td style={{...S.td,fontFamily:"monospace",fontSize:12,color:themeP,fontWeight:700}}>{b.num}</td>
                        <td style={{...S.td,fontWeight:600,color:"#E2E8F0"}}>{b.clientName}</td>
                        <td style={S.td}><span style={{padding:"2px 8px",borderRadius:20,background:st.bg,color:st.color,fontSize:11,fontWeight:700}}>{st.icon} {st.label}</span></td>
                        <td style={{...S.td,fontWeight:700,color:"#22D3A0"}}>{fmtBRL(b.total)}</td>
                        <td style={S.td}>
                          <div style={{display:"flex",gap:5,alignItems:"center"}}>
                            <button onClick={()=>gerarOrcamentoPDF(b,profile,themeP,themeA,false)} title="Exportar PDF" style={{padding:"4px 10px",borderRadius:7,background:"rgba(59,130,246,.15)",border:"1px solid rgba(59,130,246,.3)",color:"#60A5FA",cursor:"pointer",fontSize:11,fontWeight:700,fontFamily:"inherit"}}>📄 PDF</button>
                            {profile.pixChave&&<button onClick={()=>gerarOrcamentoPDF(b,profile,themeP,themeA,true)} title="Cobrar via PIX" style={{padding:"4px 10px",borderRadius:7,background:"rgba(34,211,160,.15)",border:"1px solid rgba(34,211,160,.3)",color:"#22D3A0",cursor:"pointer",fontSize:11,fontWeight:700,fontFamily:"inherit"}}>💚 Cobrar</button>}
                          </div>
                        </td>
                      </tr>
                    );
              ...
              {!profile.pixChave&&(
                <div style={{marginTop:8,padding:"8px 12px",background:"rgba(245,158,11,.06)",border:"1px solid rgba(245,158,11,.2)",borderRadius:10,fontSize:11,color:"#F59E0B"}}>
                  💡 Configure sua chave PIX em <b>Meu Perfil → PIX</b> para habilitar o botão de cobrança.
                </div>
              )}
            </div>
          )}
      ...
      {tab==="receitas"&&(
        <div>
          {rMes.length===0?(
            <div style={{textAlign:"center",padding:"48px 0",color:"#475569"}}>
              <div style={{fontSize:40,marginBottom:12}}>💚</div>
              <div style={{fontSize:14,fontWeight:600,color:"#64748B",marginBottom:8}}>Nenhuma receita manual neste mês</div>
              <button style={{...S.prim,background:"#22D3A0",color:"#0A0E1A"}} onClick={()=>setModal({type:"receita",data:null})}>+ Registrar receita</button>
            </div>
          ):(
            <div>
              <div style={{...S.card,padding:0,overflow:"hidden"}}>
                <table style={{width:"100%",borderCollapse:"collapse"}}>
                  ...
                  <tbody>{rMes.sort((a,b)=>new Date(b.date)-new Date(a.date)).map(r=>(
                    <tr key={r.id} style={{borderBottom:"1px solid #0F172A"}} className="trow">
                      <td style={{...S.td,fontSize:12,color:"#64748B"}}>{r.date}</td>
                      <td style={S.td}><span style={{padding:"2px 8px",background:"rgba(34,211,160,.1)",borderRadius:20,color:"#22D3A0",fontSize:11,fontWeight:600}}>{r.categoria}</span></td>
                      <td style={{...S.td,color:"#94A3B8"}}>{r.desc||"—"}</td>
                      <td style={{...S.td,color:"#22D3A0",fontWeight:700}}>{fmtBRL(r.valor)}</td>
                      <td style={S.td}>
                        <div style={S.acts}>
                          <TB c={themeP} t="Editar" onClick={()=>setModal({type:"receita",data:r})}>✏️</TB>
                          <TB c="#F87171" t="Excluir" onClick={()=>delReceita(r.id)}>🗑️</TB>
                        </div>
                      </td>
                    </tr>
                  ))}</tbody>
                  <tfoot><tr style={{background:"rgba(34,211,160,.05)"}}>
                    <td colSpan="3" style={{padding:"10px 14px",fontWeight:700,fontSize:13,color:"#22D3A0",borderTop:"1px solid #1E293B"}}>TOTAL</td>
                    <td style={{padding:"10px 14px",fontWeight:900,fontSize:15,color:"#22D3A0",borderTop:"1px solid #1E293B"}}>{fmtBRL(receitaManual)}</td>
                    <td style={{borderTop:"1px solid #1E293B"}}/>
                  </tr></tfoot>
                </table>
              </div>
              <div style={{marginTop:12,textAlign:"right"}}>
                <button style={{...S.prim,background:"#22D3A0",color:"#0A0E1A",fontSize:12,padding:"7px 14px"}} onClick={()=>setModal({type:"receita",data:null})}>+ Nova Receita</button>
              </div>
            </div>
          )}
        </div>
      ...
      {tab==="dre"&&(
        <div style={{maxWidth:640}}>
          <div style={S.card}>
            ...
            <button style={{...S.prim,background:"linear-gradient(135deg,#EF4444,#DC2626)",width:"100%",marginTop:16}} onClick={()=>exportDREPDF(budgets,despesas,receitas,mes,ano,profile,themeP,themeA)}>
              📄 Exportar DRE em PDF
            </button>
          </div>
        </div>
      ...
      {/* Modais */}
      {modal?.type==="despesa"&&<ModalDespesa data={modal.data} onSave={saveDespesa} onDelete={modal.data?()=>delDespesa(modal.data.id):null} onClose={()=>setModal(null)} themeP={themeP}/>}
      {modal?.type==="receita"&&<ModalReceita data={modal.data} onSave={saveReceita} onDelete={modal.data?()=>delReceita(modal.data.id):null} onClose={()=>setModal(null)} themeP={themeP}/>}
    </div>
  );
}
...
/* ═══ MODAL RECEITA ═════════════════════════════════════════════════════ */
const CATS_RECEITA=["Serviço","Venda","Comissão","Aluguel","Reembolso","Outros"];
function ModalReceita({data,onSave,onDelete,onClose,themeP}){
  const [f,sf]=useState(data||{categoria:"Serviço",desc:"",valor:0,date:today(),obs:""});
  const set=(k,v)=>sf(p=>({...p,[k]:v}));
  const ok=f.valor>0&&f.desc.trim();
  return(
    <Overlay onClose={onClose}>
      <div style={S.mhead}>
        <div><div style={S.mtitle}>{data?"Editar Receita":"Nova Receita"}</div><div style={S.msub}>Registro de entrada</div></div>
        <XBtn onClick={onClose}/>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
        <FL label="Categoria">
        ...
        <div style={{gridColumn:"1/-1"}}>
          <FL label="Descrição *"><input style={S.inp} value={f.desc} onChange={e=>set("desc",e.target.value)} placeholder="Ex: Serviço de instalação, venda de produto..."/></FL>
        </div>
        <FL label="Valor (R$) *">
          <input style={S.inp} type="number" min="0" step="0.01" value={f.valor} onChange={e=>set("valor",Number(e.target.value))}/>
        </FL>
        <FL label="Observações">
          <input style={S.inp} value={f.obs||""} onChange={e=>set("obs",e.target.value)} placeholder="Cliente, referência..."/>
        </FL>
      </div>
      <div style={{display:"flex",gap:8,marginTop:18,justifyContent:"space-between"}}>
        <div>{onDelete&&<button style={{...S.ghost,borderColor:"rgba(248,113,113,.3)",color:"#F87171",fontSize:12}} onClick={onDelete}>🗑️ Excluir</button>}</div>
        <div style={{display:"flex",gap:8}}>
          <button style={S.ghost} onClick={onClose}>Cancelar</button>
          <button style={{...S.prim,background:"#22D3A0",color:"#0A0E1A",opacity:ok?1:.4,cursor:ok?"pointer":"not-allowed"}} onClick={()=>ok&&onSave(f)}>{data?"💾 Salvar":"+ Registrar"}</button>
        </div>
      </div>
    </Overlay>
  );
...
function ModalDetail({data,onClose,setStatus,markPago,sendWA,onEdit,onDelete,themeP,setModal,profile}){
  const [del,setDel]=useState(false);
  const [showPago,setShowPago]=useState(false);
  const [pagForm,setPagForm]=useState({data:data.pagamento?.data||today(),forma:data.pagamento?.forma||"PIX",valor:data.pagamento?.valor||data.total||0,obs:data.pagamento?.obs||""});
  const sub=calcSub(data.items);const dl=daysLeft(data.date,data.validity);
  ...
  return(
    <Overlay onClose={onClose}>
      ...
      {/* Botão Marcar como Pago — disponível para aprovados e já pagos */}
      {(data.status==="aprovado"||isPago)&&(
        <div style={{marginBottom:10}}>
          {isPago&&data.pagamento&&(
            <div style={{padding:"10px 14px",background:"rgba(167,139,250,0.08)",border:"1px solid rgba(167,139,250,0.25)",borderRadius:10,marginBottom:8,fontSize:12}}>
              <div style={{fontWeight:700,color:"#A78BFA",marginBottom:4}}>💰 Pagamento registrado</div>
              <div style={{color:"#94A3B8"}}>Data: <b style={{color:"#F1F5F9"}}>{data.pagamento.data}</b> · Forma: <b style={{color:"#F1F5F9"}}>{data.pagamento.forma}</b> · Valor: <b style={{color:"#A78BFA"}}>{fmtBRL(data.pagamento.valor)}</b></div>
              {data.pagamento.obs&&<div style={{color:"#64748B",marginTop:2}}>Obs: {data.pagamento.obs}</div>}
            </div>
          )}
          {!showPago?(
            <button style={{...S.prim,width:"100%",background:"linear-gradient(135deg,#A78BFA,#7C3AED)",justifyContent:"center",display:"flex",gap:8}} onClick={()=>setShowPago(true)}>
              💰 {isPago?"Editar Pagamento":"Marcar como Pago"}
            </button>
          ):(
            <div style={{padding:"14px",background:"rgba(167,139,250,0.06)",border:"1px solid rgba(167,139,250,0.2)",borderRadius:12,marginBottom:8}}>
              <div style={{fontWeight:700,color:"#A78BFA",marginBottom:12,fontSize:13}}>💰 Registrar Pagamento</div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
                <div><div style={{fontSize:11,color:"#64748B",marginBottom:4}}>Data</div>
                  <input type="date" value={pagForm.data} onChange={e=>setPagForm(p=>({...p,data:e.target.value}))} style={{...S.inp,fontSize:12,padding:"7px 10px"}}/></div>
                <div><div style={{fontSize:11,color:"#64748B",marginBottom:4}}>Forma</div>
                  <select value={pagForm.forma} onChange={e=>setPagForm(p=>({...p,forma:e.target.value}))} style={{...S.sel,fontSize:12,padding:"7px 10px"}}>
                    {["PIX","Dinheiro","Cartão de Crédito","Cartão de Débito","Transferência","Boleto","Cheque","Outro"].map(f=><option key={f}>{f}</option>)}
                  </select></div>
              </div>
              <div style={{marginBottom:8}}><div style={{fontSize:11,color:"#64748B",marginBottom:4}}>Valor Recebido (R$)</div>
                <input type="number" value={pagForm.valor} onChange={e=>setPagForm(p=>({...p,valor:parseFloat(e.target.value)||0}))} style={{...S.inp,fontSize:12,padding:"7px 10px"}}/></div>
              <div style={{marginBottom:12}}><div style={{fontSize:11,color:"#64748B",marginBottom:4}}>Observação (opcional)</div>
                <input value={pagForm.obs} onChange={e=>setPagForm(p=>({...p,obs:e.target.value}))} placeholder="Ex: pago em 2x, desconto combinado..." style={{...S.inp,fontSize:12,padding:"7px 10px"}}/></div>
              <div style={{display:"flex",gap:8}}>
                <button style={{...S.prim,flex:1,background:"linear-gradient(135deg,#A78BFA,#7C3AED)"}} onClick={()=>{markPago(data.id,pagForm);setShowPago(false);}}>💰 Confirmar</button>
                <button style={S.ghost} onClick={()=>setShowPago(false)}>Cancelar</button>
              </div>
            </div>
          )}
        </div>
