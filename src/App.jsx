...
/* ── ADMIN ──────────────────────────────────────────────────────────────────
   checkPassword tem fallback legado que compara texto puro (sem ":").
   Para produção com backend, use hash bcrypt gerado no servidor. */
const ADMIN = {id:"admin-analua",name:"Analua",login:"analua",passwordHash:"ana0406",role:"admin"};
...
/* ═══ STORAGE (localStorage) ════════════════════════════════════════════
   Substituído window.storage (Claude-only) por localStorage padrão,
   compatível com qualquer hospedagem web (Vercel, Netlify, etc).
   O parâmetro "shared" é mantido por compatibilidade mas ignorado —
   localStorage é sempre local ao browser do usuário.               */
function useStorage(key, fallback, shared=false){
  ...
  const [loading, setL] = useState(false); // localStorage é síncrono, sem loading real
  const valRef = useRef(val);
  valRef.current = val;

  // Garante sincronização inicial (sem loading spinner desnecessário)
  useEffect(()=>{ setL(false); },[]); //eslint-disable-line

  const save = useCallback(nv=>{
    const v = typeof nv==="function" ? nv(valRef.current) : nv;
    setVal(v);
    try{ localStorage.setItem(key, JSON.stringify(v)); }catch(e){ console.warn("localStorage error:", e); }
  },[key]);
...
function AdminPanel({users,setUsers,onLogout}){
  ...
  const saveUser=async f=>{
    if(!f.id){
      ...
      // Se senha foi alterada, re-hash
      let updatedUser={...f};
      if(f._passwordChanged&&f.password){
        updatedUser={...f,password:await hashPassword(f.password),_passwordChanged:undefined};
      }
      await setUsers(us=>us.map(u=>u.id===f.id?updatedUser:u));showToast("Salvo ✓");
    }
    setModal(null);
  };
  ...
  const registerPayment=async(id)=>{
    ...
    await setUsers(us=>us.map(u=>{
      if(u.id!==id)return u;
      const hist=[...(u.payHistory||[]),{date:novaData,amount:PLANS[u.plan||"pro"]?.price||49,status:"pago"}];
      return{...u,billingStatus:"em_dia",lastPayment:novaData,nextDue:venc,payHistory:hist,active:true};
    }));
    showToast("Pagamento registrado ✓");
  };

  const markOverdue=async(id)=>{
    await setUsers(us=>us.map(u=>u.id===id?{...u,billingStatus:"atrasado"}:u));
  ...
  const stats={
    ...
    emDia:(users||[]).filter(u=>!u.billingStatus||u.billingStatus==="em_dia").length,
    atrasado:(users||[]).filter(u=>u.billingStatus==="atrasado").length,
  ...
  const bilFiltered=useMemo(()=>{
    ...
    if(bilFilter==="vencendo"){const hoje=new Date();r=r.filter(u=>{if(!u.nextDue)return false;const d=Math.ceil((new Date(u.nextDue)-hoje)/86400000);return d>=0&&d<=5;});}
        ...
        {tab==="mensalidades"&&(
          <div style={S.page}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(340px,1fr))",gap:14}}>
              {bilFiltered.map(u=>{
                const bs=u.billingStatus||"em_dia";
                const plan=PLANS[u.plan||"pro"];
                  ...
                  <div key={u.id} style={{background:"#111827",border:`1px solid ${statusColor}25`,borderRadius:18,padding:18,position:"relative",overflow:"hidden"}}>
                    ...
                    <div style={{paddingLeft:8}}>
                      ...
                      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:12}}>
                        ...
                        <div style={{background:"#0F172A",borderRadius:10,padding:"8px 10px",textAlign:"center"}}>
                          <div style={{fontSize:11,fontWeight:700,color:"#94A3B8"}}>{u.lastPayment||"—"}</div>
                          <div style={{fontSize:9,color:"#475569",textTransform:"uppercase",letterSpacing:.5}}>Último pag.</div>
                        </div>
                        <div style={{background:"#0F172A",borderRadius:10,padding:"8px 10px",textAlign:"center"}}>
                          <div style={{fontSize:11,fontWeight:700,color:vencido?"#F87171":vencendo?"#F59E0B":"#94A3B8"}}>{u.nextDue||"—"}</div>
                          <div style={{fontSize:9,color:"#475569",textTransform:"uppercase",letterSpacing:.5}}>Próx. venc.</div>
                        </div>
                      </div>

                      {(u.payHistory||[]).length>0&&(
                        <div style={{marginBottom:12}}>
                          ...
                          <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
                            {[...(u.payHistory||[])].reverse().slice(0,5).map((p,i)=>(
...
function ModalBillingDetail({data,onClose,onRegister,onWA,onOverdue}){
  ...
  const hist=[...(data.payHistory||[])].reverse();
  return(
    <Overlay onClose={onClose}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:16}}>
        <div style={{background:"#0F172A",borderRadius:12,padding:"12px 14px",border:"1px solid #1E293B"}}>
          ...
          <div style={{fontSize:15,fontWeight:700,color:"#22D3A0"}}>{data.lastPayment||"Nenhum"}</div>
        </div>
        <div style={{background:"#0F172A",borderRadius:12,padding:"12px 14px",border:"1px solid #1E293B"}}>
          <div style={{fontSize:10,color:"#64748B",textTransform:"uppercase",letterSpacing:.5,marginBottom:4}}>Próximo vencimento</div>
          <div style={{fontSize:15,fontWeight:700,color:data.billingStatus==="atrasado"?"#F87171":"#F59E0B"}}>{data.nextDue||"—"}</div>
        </div>
      </div>
...
function ModalUserForm({data,onSave,onClose}){
  ...
  const [showP,setShowP]=useState(false);const [showP2,setShowP2]=useState(false);
  const [passMsg,setPassMsg]=useState(null); // {type:"ok"|"err", text}
  const set=(k,v)=>sf(p=>({...p,[k]:v}));
  const ok=f.name&&f.login&&(data||f.password);

  const applyPassword=()=>{
    if(newPass.length<4){setPassMsg({type:"err",text:"Senha deve ter ao menos 4 caracteres."});return;}
    if(newPass!==newPass2){setPassMsg({type:"err",text:"As senhas não coincidem."});return;}
    sf(p=>({...p,password:newPass,_passwordChanged:true}));
    setPassMsg({type:"ok",text:"Senha atualizada! Clique em Salvar para confirmar."});
    setNewPass("");setNewPass2("");
  };

  const TABS=data
    ?[{id:"dados",lbl:"👤 Dados"},{id:"senha",lbl:"🔐 Senha"}]
    :[{id:"dados",lbl:"👤 Dados"}];

  return(
    <Overlay onClose={onClose}>
      <div style={S.mhead}><div><div style={S.mtitle}>{data?"Editar Usuário":"Criar Usuário"}</div>{data&&<div style={S.msub}>{data.login}</div>}</div><XBtn onClick={onClose}/></div>

      {/* tabs — só mostra quando editando */}
      {data&&(
        <div style={{display:"flex",gap:4,background:"#0F172A",borderRadius:10,padding:4,marginBottom:18,border:"1px solid #1E293B"}}>
          {TABS.map(t=>(
            <button key={t.id} onClick={()=>setTab(t.id)} style={{flex:1,padding:"8px",border:"none",borderRadius:7,cursor:"pointer",fontSize:12,fontFamily:"inherit",fontWeight:tab===t.id?700:500,background:tab===t.id?"#1E293B":"transparent",color:tab===t.id?"#F1F5F9":"#64748B",transition:"all .2s"}}>{t.lbl}</button>
          ))}
        </div>
      )}
      ...
      {tab==="dados"&&(
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          ...
          {!data&&(
            <FL label="Senha *">
              <div style={{position:"relative"}}>
                <input style={{...S.inp,paddingRight:36}} type={showP?"text":"password"} value={f.password} onChange={e=>set("password",e.target.value)} placeholder="Mín. 4 caracteres"/>
                <button onClick={()=>setShowP(s=>!s)} style={{position:"absolute",right:8,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",fontSize:13}}>{showP?"🙈":"👁️"}</button>
              </div>
            </FL>
          )}
      ...
      {/* ABA SENHA (admin reseta senha do usuário) */}
      {tab==="senha"&&(
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          <div style={{padding:"12px 14px",background:"rgba(129,140,248,0.06)",borderRadius:10,border:"1px solid rgba(129,140,248,0.15)",fontSize:12,color:"#818CF8"}}>
            🔐 Como administradora, você pode redefinir a senha deste usuário sem precisar da senha atual.
          </div>
          <FL label="Nova senha">
            <div style={{position:"relative"}}>
              <input style={{...S.inp,paddingRight:36}} type={showP?"text":"password"} value={newPass} onChange={e=>{setNewPass(e.target.value);setPassMsg(null);}} placeholder="Mín. 4 caracteres"/>
              <button onClick={()=>setShowP(s=>!s)} style={{position:"absolute",right:8,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",fontSize:13}}>{showP?"🙈":"👁️"}</button>
            </div>
          </FL>
          <FL label="Confirmar nova senha">
            <div style={{position:"relative"}}>
              <input style={{...S.inp,paddingRight:36}} type={showP2?"text":"password"} value={newPass2} onChange={e=>{setNewPass2(e.target.value);setPassMsg(null);}} placeholder="Repita a senha"/>
              <button onClick={()=>setShowP2(s=>!s)} style={{position:"absolute",right:8,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",fontSize:13}}>{showP2?"🙈":"👁️"}</button>
            </div>
          </FL>
          {passMsg&&(
            <div style={{display:"flex",alignItems:"center",gap:8,padding:"9px 12px",borderRadius:9,fontSize:12,fontWeight:600,background:passMsg.type==="ok"?"rgba(34,211,160,0.08)":"rgba(248,113,113,0.08)",color:passMsg.type==="ok"?"#22D3A0":"#F87171",border:`1px solid ${passMsg.type==="ok"?"rgba(34,211,160,0.2)":"rgba(248,113,113,0.2)"}`}}>
              {passMsg.type==="ok"?"✅":"⚠️"} {passMsg.text}
            </div>
          )}
          <button style={{...S.prim,background:"linear-gradient(135deg,#818CF8,#6366F1)"}} onClick={applyPassword}>🔐 Aplicar nova senha</button>
          {f._passwordChanged&&<div style={{fontSize:11,color:"#F59E0B",textAlign:"center"}}>⚠️ Senha alterada — clique em Salvar para confirmar</div>}
        </div>
      )}
...
/* ═══ APP SHELL ══════════════════════════════════════════════════════════ */
function AppShell({user,users,setUsers,onLogout}){
  ...
  return<App user={user} data={{...data,profile}} patch={patch} setUsers={setUsers} themeP={P} themeA={A} onLogout={onLogout}/>;
}

/* ═══ APP ════════════════════════════════════════════════════════════════ */
function App({user,data,patch,setUsers,themeP,themeA,onLogout}){
  const {budgets=[],clients=[],templates=[],profile={},activity=[]}=data;
  ...
  const NAVS=[{id:"dashboard",ico:"📊",lbl:"Dashboard"},{id:"lista",ico:"📋",lbl:"Orçamentos"},{id:"clientes",ico:"👥",lbl:"Clientes"},{id:"relatorio",ico:"📈",lbl:"Relatórios"},{id:"templates",ico:"📄",lbl:"Templates"},{id:"atividade",ico:"🕐",lbl:"Atividades"},{id:"config",ico:"⚙️",lbl:"Meu Perfil"}];
  const MOBILE_NAVS=NAVS.slice(0,5); // dashboard, lista, clientes, relatorio, config

  return(
    <div style={S.root}>
      {/* SIDEBAR — desktop only */}
      <aside className="desktop-sidebar" style={{...S.side,width:sideOpen?246:68,transition:"width .3s cubic-bezier(.4,0,.2,1)",borderRight:`1px solid ${themeP}18`}}>
        <div style={S.sTop}>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        {/* TOPBAR — desktop */}
        <div className="hide-mobile" style={{...S.topbar,borderBottom:`1px solid ${themeP}15`}}>
        ...
        {/* TOPBAR — mobile */}
        <div className="mobile-topbar" style={{height:54,background:"#0D1320",borderBottom:`1px solid ${themeP}20`,alignItems:"center",justifyContent:"space-between",padding:"0 16px",flexShrink:0,display:"none"}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <div style={{width:28,height:28,background:`linear-gradient(135deg,${themeP},${themeA})`,borderRadius:7,display:"flex",alignItems:"center",justifyContent:"center",fontSize:14}}>⚡</div>
            <span style={{fontWeight:800,fontSize:15,color:"#F1F5F9"}}>{profile.name||"OrcaPro"}</span>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            <button style={{...S.prim,padding:"6px 14px",fontSize:12,background:`linear-gradient(135deg,${themeP},${themeA})`}} onClick={()=>setModal({type:"budget",data:null})}>+ Novo</button>
            <button onClick={()=>setPage("config")} style={{background:"none",border:"none",cursor:"pointer"}}>
              {profile.logo?<img src={profile.logo} alt="" style={{width:28,height:28,borderRadius:"50%",objectFit:"cover"}}/>:<Ava name={user.name} size={28} color={themeP}/>}
            </button>
          </div>
        </div>
        <main className="page-main" style={{flex:1,overflowY:"auto",background:"#0A0E1A"}}>
          ...
          {page==="config"    &&<PageConfig profile={profile} setProfile={setProfile} user={user} setUsers={setUsers} themeP={themeP} themeA={themeA} showToast={showToast}/>}
        </main>
      </div>
      {/* BOTTOM NAV — mobile */}
      <nav className="mobile-bottomnav" style={{position:"fixed",bottom:0,left:0,right:0,height:64,background:"#0D1320",borderTop:`1px solid ${themeP}20`,display:"none",alignItems:"center",justifyContent:"space-around",zIndex:50,padding:"0 4px"}}>
        {[{id:"dashboard",ico:"📊",lbl:"Início"},{id:"lista",ico:"📋",lbl:"Orçamentos"},{id:"clientes",ico:"👥",lbl:"Clientes"},{id:"relatorio",ico:"📈",lbl:"Relatório"},{id:"config",ico:"⚙️",lbl:"Perfil"}].map(n=>(
          <button key={n.id} onClick={()=>setPage(n.id)} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:2,background:"none",border:"none",cursor:"pointer",fontFamily:"inherit",padding:"6px 8px",borderRadius:10,flex:1,transition:"all .2s",color:page===n.id?themeP:"#475569"}}>
            <span style={{fontSize:20}}>{n.ico}</span>
            <span style={{fontSize:9,fontWeight:page===n.id?700:500}}>{n.lbl}</span>
            {page===n.id&&<span style={{width:4,height:4,borderRadius:"50%",background:themeP}}/>}
          </button>
        ))}
      </nav>
...
function PageConfig({profile,setProfile,user,setUsers,themeP,themeA,showToast}){
  ...
  const changePassword=async()=>{
    setPassMsg(null);
    if(!curPass){setPassMsg({t:"err",v:"Informe sua senha atual."});return;}
    const ok=await checkPassword(curPass,user.password);
    if(!ok){setPassMsg({t:"err",v:"Senha atual incorreta."});return;}
    if(newPass.length<4){setPassMsg({t:"err",v:"Nova senha deve ter ao menos 4 caracteres."});return;}
    if(newPass!==newPass2){setPassMsg({t:"err",v:"As novas senhas não coincidem."});return;}
    const hash=await hashPassword(newPass);
    if(setUsers) setUsers(us=>us.map(u=>u.id===user.id?{...u,password:hash}:u));
    setPassMsg({t:"ok",v:"Senha alterada com sucesso! ✅"});
  ...
  const TABS=[{id:"dados",lbl:"👤 Dados"},{id:"visual",lbl:"🎨 Identidade Visual"},{id:"orcamento",lbl:"📄 Orçamento"},{id:"social",lbl:"🌐 Links"},{id:"senha",lbl:"🔐 Senha"}];
  ...
  return(
    <div style={S.page}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr",gap:14,maxWidth:680}}>
        ...
        {tab==="senha"&&(
          <React.Fragment>
            <Card title="🔐 Alterar Senha">
              <div style={{display:"flex",flexDirection:"column",gap:14,marginTop:12}}>
                <div style={{padding:"10px 14px",background:"rgba(129,140,248,0.06)",borderRadius:10,border:"1px solid rgba(129,140,248,0.15)",fontSize:12,color:"#818CF8"}}>
                  Para alterar sua senha, confirme a senha atual primeiro.
                </div>
                <FL label="Senha atual">
                  <div style={{position:"relative"}}>
                    <input style={{...S.inp,paddingRight:36}} type={showCP?"text":"password"} value={curPass} onChange={e=>{setCurPass(e.target.value);setPassMsg(null);}} placeholder="Sua senha atual"/>
                    <button onClick={()=>setShowCP(s=>!s)} style={{position:"absolute",right:8,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",fontSize:13}}>{showCP?"🙈":"👁️"}</button>
                  </div>
                </FL>
                <FL label="Nova senha">
                  <div style={{position:"relative"}}>
                    <input style={{...S.inp,paddingRight:36}} type={showNP?"text":"password"} value={newPass} onChange={e=>{setNewPass(e.target.value);setPassMsg(null);}} placeholder="Mínimo 4 caracteres"/>
                    <button onClick={()=>setShowNP(s=>!s)} style={{position:"absolute",right:8,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",fontSize:13}}>{showNP?"🙈":"👁️"}</button>
                  </div>
                </FL>
                <FL label="Confirmar nova senha">
                  <div style={{position:"relative"}}>
                    <input style={{...S.inp,paddingRight:36}} type={showNP2?"text":"password"} value={newPass2} onChange={e=>{setNewPass2(e.target.value);setPassMsg(null);}} placeholder="Repita a nova senha"/>
                    <button onClick={()=>setShowNP2(s=>!s)} style={{position:"absolute",right:8,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",fontSize:13}}>{showNP2?"🙈":"👁️"}</button>
                  </div>
                </FL>
                {passMsg&&(
                  <div style={{display:"flex",alignItems:"center",gap:8,padding:"10px 14px",borderRadius:9,fontSize:13,fontWeight:600,background:passMsg.t==="ok"?"rgba(34,211,160,0.08)":"rgba(248,113,113,0.08)",color:passMsg.t==="ok"?"#22D3A0":"#F87171",border:`1px solid ${passMsg.t==="ok"?"rgba(34,211,160,0.25)":"rgba(248,113,113,0.25)"}`}}>
                    {passMsg.t==="ok"?"✅":"⚠️"} {passMsg.v}
                  </div>
                )}
                <button style={{...S.prim,background:`linear-gradient(135deg,${themeP},${themeA})`,padding:"12px"}} onClick={changePassword}>
                  🔐 Alterar senha
                </button>
              </div>
            </Card>
        ...
        {tab!=="senha"&&<button style={{...S.prim,padding:"12px",fontSize:14,background:`linear-gradient(135deg,${themeP},${themeA})`,boxShadow:`0 4px 16px ${themeP}30`}} onClick={save}>
...
function PageDash({stats,budgets,user,profile,clients,themeP,themeA,setModal,setStatus,sendWA,setPage}){
  ...
  return(
    <div style={S.page}>
      ...
      <div className="dash-grid" style={{display:"grid",gridTemplateColumns:"1fr 300px",gap:20}}>
...
function PageRelatorio({budgets,stats,clients}){
  ...
  return(
    <div style={S.page}>
      ...
      <div className="grid-2 report-grid" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:16}}>
...
function ModalDetail({data,onClose,setStatus,sendWA,onEdit,onDelete,themeP}){
  ...
  return(
    <Overlay onClose={onClose}>
      ...
      <div style={{display:"flex",gap:8}}>
        ...
        <button style={{...S.ghost,padding:"8px 14px",borderColor:"rgba(248,113,113,.3)",color:"#F87171"}} onClick={()=>setDel(true)}>🗑️</button>
      </div>
...
const Overlay=({onClose,children,wide})=>{
  ...
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.8)",backdropFilter:"blur(6px)",display:"flex",alignItems:mob?"flex-end":"center",justifyContent:"center",zIndex:100,padding:mob?0:16}} onClick={onClose}>
      <div style={{background:"#111827",border:"1px solid #1E293B",borderRadius:mob?"20px 20px 0 0":20,padding:mob?"20px 16px 32px":26,width:"100%",maxWidth:mob?"100%":wide?680:520,maxHeight:mob?"92vh":"92vh",overflowY:"auto"}} onClick={e=>e.stopPropagation()}>{children}</div>
    </div>
  );
};
...
/* ═══ STYLES ═════════════════════════════════════════════════════════════ */
const S={
  root:    {display:"flex",height:"100vh",width:"100vw",background:"#0A0E1A",fontFamily:"'DM Sans',sans-serif",color:"#CBD5E1",overflow:"hidden"},
  ...
  page:    {padding:"24px 24px 60px",minHeight:"100%"},
...
const L={
  root:    {display:"flex",height:"100vh",width:"100vw",alignItems:"center",justifyContent:"center",background:"#060912",fontFamily:"'DM Sans',sans-serif",position:"relative",overflow:"hidden"},
  ...
  wrap:    {background:"rgba(13,19,32,0.96)",border:"1px solid #1E293B",borderRadius:28,padding:40,width:"100%",maxWidth:420,position:"relative",zIndex:1,boxShadow:"0 32px 80px rgba(0,0,0,.7)",margin:"0 12px"},
...
.page-main{flex:1;overflow-y:auto;background:#0A0E1A;}
@media(max-width:768px){.page-main>div{padding-bottom:80px!important;}}
...
@media(max-width:768px){
  ...
  .page-content{padding:14px 12px 90px!important;}
  ...
  .modal-full{max-width:100%!important;max-height:100vh!important;border-radius:20px 20px 0 0!important;margin-top:auto;}
  .overlay-mobile{align-items:flex-end!important;padding:0!important;}
  .budget-form-grid{grid-template-columns:1fr!important;}
  .items-grid{grid-template-columns:1fr 48px 56px 80px 28px!important;}
  .report-grid{grid-template-columns:1fr!important;}
  .login-wrap{border-radius:20px!important;padding:28px 20px!important;margin:12px!important;}
}
