...
function App({user,data,patch,themeP,themeA,onLogout}){
  ...
  const modais=(
    <React.Fragment>
      {modal?.type==="budget"   &&(modal.data?
        <ModalBudget data={modal.data} clients={clients} templates={templates} onSave={saveBudget} onClose={()=>setModal(null)} nextNum={nextNum} userId={user.id} themeP={themeP} themeA={themeA}/>:
        modal.mode==="manual"?<ModalBudget data={null} clients={clients} templates={templates} onSave={saveBudget} onClose={()=>setModal(null)} nextNum={nextNum} userId={user.id} themeP={themeP} themeA={themeA}/>:
        modal.mode==="ia"?<ModalBudgetIA clients={clients} onSave={saveBudget} onClose={()=>setModal(null)} nextNum={nextNum} userId={user.id} themeP={themeP} themeA={themeA}/>:
        <ModalBudgetMode onManual={()=>setModal({type:"budget",mode:"manual"})} onIA={()=>setModal({type:"budget",mode:"ia"})} onClose={()=>setModal(null)} themeP={themeP} themeA={themeA}/>
      )}
...
function ModalBudgetMode({onManual,onIA,onClose,themeP,themeA}){
  return(
    <Overlay onClose={onClose}>
      <div style={S.mhead}>
        <div><div style={S.mtitle}>Novo Orçamento</div><div style={S.msub}>Como deseja criar?</div></div>
        <XBtn onClick={onClose}/>
      </div>
      <div style={{display:"flex",flexDirection:"column",gap:14,padding:"8px 0 16px"}}>
        {/* IA Option */}
        <button onClick={onIA} style={{background:"linear-gradient(135deg,rgba(129,140,248,.12),rgba(34,211,160,.08))",border:"2px solid rgba(129,140,248,.4)",borderRadius:18,padding:"22px 20px",cursor:"pointer",textAlign:"left",fontFamily:"inherit",transition:"all .2s",display:"flex",gap:16,alignItems:"flex-start"}}>
          <div style={{fontSize:36,flexShrink:0}}>🤖</div>
          <div>
            <div style={{fontSize:15,fontWeight:900,color:"#818CF8",marginBottom:5}}>Gerar com Inteligência Artificial</div>
            <div style={{fontSize:12,color:"#64748B",lineHeight:1.6}}>Descreva o serviço em linguagem normal e a IA monta o orçamento completo com itens, quantidades e preços sugeridos.</div>
            <div style={{marginTop:10,display:"flex",gap:6,flexWrap:"wrap"}}>
              {["⚡ Rápido","✅ Itens automáticos","💰 Preços sugeridos"].map(t=>(
                <span key={t} style={{fontSize:10,padding:"3px 8px",background:"rgba(129,140,248,.1)",borderRadius:20,color:"#818CF8",fontWeight:600}}>{t}</span>
              ))}
            </div>
          </div>
        </button>
        ...
        {/* Manual Option */}
        <button onClick={onManual} style={{background:"#0F172A",border:"2px solid #1E293B",borderRadius:18,padding:"22px 20px",cursor:"pointer",textAlign:"left",fontFamily:"inherit",transition:"all .2s",display:"flex",gap:16,alignItems:"flex-start"}}>
          <div style={{fontSize:36,flexShrink:0}}>✍️</div>
          <div>
            <div style={{fontSize:15,fontWeight:900,color:"#F1F5F9",marginBottom:5}}>Criar Manualmente</div>
            <div style={{fontSize:12,color:"#64748B",lineHeight:1.6}}>Preencha os dados do orçamento passo a passo com controle total sobre itens e valores.</div>
            <div style={{marginTop:10,display:"flex",gap:6,flexWrap:"wrap"}}>
              {["🎯 Controle total","📄 Templates","🔧 Personalizado"].map(t=>(
                <span key={t} style={{fontSize:10,padding:"3px 8px",background:"rgba(255,255,255,.04)",borderRadius:20,color:"#64748B",fontWeight:600}}>{t}</span>
              ))}
            </div>
          </div>
        </button>
...
/* ═══ MODAL BUDGET IA ═══════════════════════════════════════════════════ */
function ModalBudgetIA({clients,onSave,onClose,nextNum,userId,themeP,themeA}){
  const [step,setStep]=useState(0); // 0=prompt, 1=loading, 2=review
  ...
  const EXEMPLOS=[
    "Instalar 5 tomadas e 3 interruptores em casa",
    "Pintar sala e 2 quartos com massa corrida",
    "Consultoria de marketing digital por 1 mês",
    "Reformar banheiro completo com troca de revestimento",
    "Instalar sistema de câmeras CCTV com 4 câmeras",
  ];
  ...
  const si=(i,k,v)=>setEditItems(prev=>{const n=[...prev];n[i]={...n[i],[k]:v};return n;});
  const addItem=()=>setEditItems(p=>[...p,{id:uid(),desc:"",qty:1,unit:"un",price:0}]);
  const rmItem=i=>setEditItems(p=>p.filter((_,j)=>j!==i));
  const total=editItems.reduce((s,i)=>s+(Number(i.qty)||0)*(Number(i.price)||0),0);

  const salvar=()=>{
    onSave({
      clientName,clientId,phone,email:"",
      category:generated.category||"Outros",
      title:generated.title,
      desc:generated.desc||"",
      items:editItems,
      status:"pendente",
      date:today(),
      validity:generated.validity||15,
      discount:0,tax:0,
  ...
  return(
    <Overlay onClose={onClose} wide>
      <div style={S.mhead}>
        <div>
          <div style={S.mtitle}>🤖 Orçamento com IA</div>
          <div style={S.msub}>{step===0?"Descreva o serviço":step===1?"Gerando...":"Revise e salve"} · {nextNum}</div>
        </div>
        <XBtn onClick={onClose}/>
      </div>
      ...
      <div style={{display:"flex",gap:4,marginBottom:20}}>
        {["Descrição","Gerando","Revisão"].map((s,i)=>(
          <div key={s} style={{flex:1,textAlign:"center"}}>
            <div style={{height:3,borderRadius:2,background:i<=step?`linear-gradient(90deg,${themeP},${themeA})`:"#1E293B"}}/>
            <div style={{fontSize:10,color:i===step?themeP:"#475569",marginTop:3,fontWeight:i===step?700:400}}>{s}</div>
          </div>
        ))}
      </div>
      ...
      {step===0&&(
        <div style={{display:"flex",flexDirection:"column",gap:16}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            <div style={{gridColumn:"1/-1"}}>
              <FL label="Cliente *">
                <input style={S.inp} value={clientName} onChange={e=>setClientName(e.target.value)} placeholder="Nome do cliente"/>
              </FL>
            </div>
            <FL label="WhatsApp"><input style={S.inp} value={phone} onChange={e=>setPhone(e.target.value)} placeholder="11999990000"/></FL>
          </div>
          {clients.length>0&&(
            <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
              {clients.map(c=>(
                <button key={c.id} style={{...S.chip2,...(clientId===c.id?{borderColor:themeP,color:themeP,background:`${themeP}10`}:{})}} onClick={()=>{setClientName(c.name);setClientId(c.id);setPhone(c.phone||"");}}>
                  <Ava name={c.name} size={16} color={themeP}/>{c.name}
                </button>
              ))}
            </div>
          )}
          ...
          {/* Prompt */}
          <FL label="Descreva o serviço *">
            <textarea
              style={{...S.inp,minHeight:90,resize:"vertical",fontSize:14,lineHeight:1.6}}
              value={prompt}
              onChange={e=>setPrompt(e.target.value)}
              placeholder="Ex: Instalar 3 tomadas e 2 interruptores em apartamento de 2 quartos em São Paulo..."
            />
          </FL>

          {/* Exemplos */}
          <div>
            <div style={{fontSize:10,color:"#475569",fontWeight:700,textTransform:"uppercase",letterSpacing:.7,marginBottom:8}}>Exemplos rápidos</div>
            <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
              {EXEMPLOS.map(e=>(
                <button key={e} style={{...S.chip2,fontSize:11}} onClick={()=>setPrompt(e)}>{e}</button>
              ))}
            </div>
          </div>

          {error&&<div style={{fontSize:12,color:"#F87171",padding:"8px 12px",background:"rgba(248,113,113,.08)",borderRadius:8}}>⚠️ {error}</div>}

          <div style={{display:"flex",justifyContent:"space-between",marginTop:4}}>
            <button style={S.ghost} onClick={onClose}>Cancelar</button>
            <button
              style={{...S.prim,background:`linear-gradient(135deg,${themeP},${themeA})`,opacity:(prompt.trim()&&clientName.trim())?1:.4,cursor:(prompt.trim()&&clientName.trim())?"pointer":"not-allowed"}}
              onClick={()=>{if(prompt.trim()&&clientName.trim())gerar();}}>
              🤖 Gerar Orçamento →
            </button>
          </div>
        </div>
      ...
      {step===1&&(
        <div style={{textAlign:"center",padding:"40px 0"}}>
          ...
          <div style={{fontSize:16,fontWeight:700,color:"#F1F5F9",marginBottom:8}}>Gerando seu orçamento...</div>
          <div style={{fontSize:13,color:"#64748B"}}>A IA está montando os itens e sugerindo preços</div>
      ...
      {step===2&&generated&&(
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          {/* Dados gerados */}
          <div style={{padding:"12px 14px",background:"rgba(129,140,248,.06)",borderRadius:12,border:"1px solid rgba(129,140,248,.2)"}}>
            <div style={{fontSize:11,color:"#818CF8",fontWeight:700,textTransform:"uppercase",letterSpacing:.7,marginBottom:6}}>✨ Gerado pela IA</div>
            <div style={{fontSize:14,fontWeight:700,color:"#F1F5F9",marginBottom:3}}>{generated.title}</div>
            <div style={{fontSize:11,color:"#64748B"}}>{generated.category} · {generated.desc}</div>
          </div>

          {/* Itens editáveis */}
          <div>
            <div style={{fontSize:11,color:"#475569",fontWeight:700,textTransform:"uppercase",letterSpacing:.7,marginBottom:8}}>Itens — edite se necessário</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 54px 64px 90px 28px",gap:6,marginBottom:6}}>
              {["Descrição","Qtd","Unid","Valor (R$)",""].map(h=><div key={h} style={{fontSize:9,color:"#475569",fontWeight:700,textTransform:"uppercase"}}>{h}</div>)}
            </div>
            {editItems.map((it,i)=>(
              <div key={it.id} style={{display:"grid",gridTemplateColumns:"1fr 54px 64px 90px 28px",gap:6,marginBottom:6}}>
                <input style={S.inp} value={it.desc} onChange={e=>si(i,"desc",e.target.value)}/>
                ...
                <input style={S.inp} type="number" step=".01" value={it.price} onChange={e=>si(i,"price",e.target.value)}/>
            ...
            <button style={{...S.ghost,fontSize:12,padding:"6px 12px",marginTop:4}} onClick={addItem}>+ Item</button>
          </div>

          {/* Total */}
          <div style={{display:"flex",justifyContent:"flex-end",paddingTop:8,borderTop:"1px solid #1E293B"}}>
            <div style={{fontSize:22,fontWeight:900,color:themeP}}>{fmtBRL(total)}</div>
          </div>

          {generated.notes&&(
            <div style={{fontSize:12,color:"#64748B",padding:"8px 12px",background:"#0F172A",borderRadius:8,border:"1px solid #1E293B"}}>
              📋 {generated.notes}
            </div>
          )}
          ...
          <div style={{display:"flex",justifyContent:"space-between",marginTop:4,gap:8}}>
            <div style={{display:"flex",gap:8}}>
              <button style={S.ghost} onClick={()=>setStep(0)}>← Refazer</button>
              <button style={S.ghost} onClick={onClose}>Cancelar</button>
            </div>
            <button style={{...S.prim,background:`linear-gradient(135deg,${themeP},${themeA})`}} onClick={salvar}>
              ✅ Salvar Orçamento
            </button>
          </div>
        </div>
